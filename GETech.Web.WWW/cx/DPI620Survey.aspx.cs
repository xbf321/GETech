...
public partial class cx_DPI620Survey : System.Web.UI.Page
{
    protected string trackId = string.Empty;
    protected string errorTip = "非法请求！<a href=\"/cx/dpi620.aspx\">返回</a>";
        ...
        if (!string.IsNullOrEmpty(key))
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    {
                        //已经填写
                        Response.Write("您已经填写过此调查了，请等待工作人员和您联系，<a href=\"/cx/dpi620.aspx\">返回</a>");
                        Response.End();
                    }
                }
            }
    ...
    void btnOK_Click(object sender, EventArgs e)
    {
        ...
        if (tipMessage.Length > 0 && error)
        {
            ...
            string q2Select = string.Empty;
            foreach(ListItem item in q2.Items){
                if (item.Selected) {
                    q2Select += item.Value + ",";
                }
            }
            insertSurvey(guid,"q2",q2Select,"checkbox");
            insertSurvey(guid, "q3", this.q3.SelectedValue, "radio");
