...
public partial class cx_DPI620 : System.Web.UI.Page
{
    ...
    {
        /*Page.ClientScript.RegisterStartupScript(this.GetType(), "ShowSuccessDialog", "<script type=\"text/javascript\">showSuccessDialog()</script>");
        */
        ...
        if(this.cbP1.Checked){
            ...
            selectProductForEmail.AppendFormat("买【德鲁克DPI620多功能过程信号校验仪】，购买台数：{0}，大致购买时间 ：{1}。<br />",this.ddlP1PC.SelectedValue,this.txtP1DateTime.Text);
        }
        if(this.cbP2.Checked){
            selectProduct.AppendFormat(",P2-{0}-{1}", this.ddlP2PC.SelectedValue, this.txtP2DateTime.Text);
            selectProductForEmail.AppendFormat("买【德鲁克DPI620的压力基座和压力模块】，购买台数：{0}，大致购买时间 ：{1}。<br />", this.ddlP2PC.SelectedValue, this.txtP2DateTime.Text);
        }
        if (this.cbP3.Checked)
        {
            selectProduct.AppendFormat(",P3-{0}-{1}", this.ddlP3PC.SelectedValue, this.txtP3DateTime.Text);
            selectProductForEmail.AppendFormat("买【德鲁克DPI610/615系列】，购买台数：{0}，大致购买时间 ：{1}。<br />", this.ddlP3PC.SelectedValue, this.txtP3DateTime.Text);
        }

        //注册
        string strSQL = "INSERT INTO Promotion201204(GUID,ProductType,RealName,Mobile,Company,Position,Department,Phone,Email,Source,SelectProduct) VALUES(@GUID,'DPI',@RealName,@Mobile,@Company,@Position,@Department,@Phone,@Email,@Source,@SelectProduct);SELECT @@IDENTITY;";
        ...
        {
            //生成追踪号 格式DPI04XXXXX
            trackId = string.Format("DPI{1}{0}", i.ToString("0000"),DateTime.Now.ToString("MMdd"));
            //更新追踪号
    ...
    void SendEmail(string email,string body)
    {
        ...
        string[] cc = { "Dishu.Sun@ge.com", "sensing.cn@ge.com", "shelley.hou@ge.com", "hensen.zhang@ge.com", "slc007@163.com" };
